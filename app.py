...
def processar_csv(arquivo):
    try:
        ...
        if colunas_faltantes:
            st.error(
                "Colunas obrigatorias faltando: " + ", ".join(colunas_faltantes) +
        ...
        for idx, row in df.iterrows():
            try:
                ...
                erros.append("Linha " + str(idx + 2) + ": " + str(e))

        if erros:
            with st.expander(str(len(erros)) + " linha(s) com erro (ignoradas)"):
                for erro in erros:
                    st.warning(erro)
...
if opcao == "📤 Upload de Vendas":
    ...
    st.code("cliente,valor,data\nJoao Silva,1500.00,2025-01-15\nMaria Santos,2300.50,2025-01-20", language="csv")
    ...
    if arquivo is not None:
        ...
        if df is not None:
            ...
            st.success("✅ Arquivo processado com sucesso! " + str(len(df)) + " registros validos.")
...
# ============================================
    ...
    if st.session_state.dados_vendas is None:
        ...
        with col1:
            st.info("**Clientes listados:** " + str(len(consolidado_display_filtrado)))
        with col2:
            total_pontos = consolidado['Pontos Disponiveis (R$)'].sum()
            st.info("**Total de pontos:** " + formatar_moeda(total_pontos))
        with col3:
            total_gasto = consolidado['Total Gasto (R$)'].sum()
            st.info("**Total gasto:** " + formatar_moeda(total_gasto))

        csv_export = consolidado.to_csv(index=False).encode('utf-8-sig')
        st.download_button(
            label="📥 Exportar lista de clientes (CSV)",
            data=csv_export,
            file_name="clientes_pontos_" + datetime.now().strftime('%Y%m%d') + ".csv",
            mime="text/csv"
        )
...
# ============================================
    ...
    if st.session_state.dados_vendas is None:
        ...
        if expirados.empty:
            ...
            st.error("⚠️ " + str(len(expirados)) + " registro(s) com pontos expirados")
...
# ============================================
    ...
    st.markdown("O arquivo deve conter as colunas: **cliente**, **valor**, **data**")
    st.markdown(
        "**Aceita variacoes:**\n"
        "- Valores com virgula decimal: 1500,00\n"
        "- Valores com R$: R$ 1.500,00\n"
        "- Datas em varios formatos: 2025-01-15, 15/01/2025, 15-01-2025"
    )
